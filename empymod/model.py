...
def bipole(src, rec, depth, res, freqtime, signal=None, aniso=None,
    ...
    msrc, mrec : bool or string, default: False
        ...
        - ``'b'``: Source and receiver can also be set to ``msrc='b'`` and
          ...
          magnetic flux density B instead of magnetic field H, with B=μH. It
          computes the same as ``mrec=True``, but multiplies it with μ of the
          layer of the receiver in the frequency domain (and accordingly four
          the source). Only implemented for isotropic magnetic permittivities
          at source/receiver level.

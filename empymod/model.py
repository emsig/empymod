...
__all__ = ['bipole', 'dipole', 'cole_cole', 'analytical', 'gpr', 'dipole_k',
           'fem', 'tem', 'wavenumber']
...
def cole_cole(src, rec, depth, cond_0, cond_8, cond_tau, cond_c, freqtime,
              signal=None, aniso=None, epermH=None, epermV=None, mpermH=None,
              mpermV=None, msrc=False, srcpts=1, mrec=False, recpts=1,
              strength=0, xdirect=False, ht='fht', htarg=None, ft='sin',
              ftarg=None, opt=None, loop=None, verb=2):
    """Return the electromagnetic field due to an electromagnetic source.
    ...
    # Check layer parameters
    model = check_model(depth, cond_0, aniso, epermH, epermV, mpermH, mpermV,
                        xdirect, verb)
    depth, res, aniso, epermH, epermV, mpermH, mpermV, isfullspace = model
    ...
    # # # v v v v v   NEW v v v v v # # #
    from empymod.utils import _check_var
    cond_0 = _check_var(cond_0, float, 1, 'cond_0', depth.shape)
    cond_8 = _check_var(cond_8, float, 1, 'cond_8', depth.shape)
    ...
    # We are now going to recalculate etaH, only using the real part of the
    # above, because that's the part from el. permittivity

    # Get complex conductivity from cole-cole
    comp_cond = (cond_0-cond_8)/(1 + np.outer(1j*np.pi*freq, cond_tau)**cond_c)
    comp_cond += cond_8

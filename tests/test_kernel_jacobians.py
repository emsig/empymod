...
def test_reflections_jacobian(njit):  # 3. reflections
    ...
    Rp, Rm, dRp, dRm = reflections(depth, e_zH, Gam, lrec, lsrc, ana_deriv=True, dGam=dGam, debug=True)
    ...
    for cond_idx in np.arange(cond.size):
        ...
        gamma2 = z_eH * e_zH
        Gam = np.sqrt(kappa ** 2 + gamma2).reshape(1, 1, -1, 1)
        Rp_pert, Rm_pert = reflections(depth, e_zH, Gam, lrec, lsrc, ana_deriv=False, debug=True)
        ...
        gamma2 = z_eH * e_zH
        Gam = np.sqrt(kappa ** 2 + gamma2).reshape(1, 1, -1, 1)
        Rp_pert2, Rm_pert2 = reflections(depth, e_zH, Gam, lrec, lsrc, ana_deriv=False, debug=True)
...
def test_fields_jacobian(njit):
    ...
    Gam = np.sqrt(kappa ** 2 + gamma2).reshape(1, 1, -1, 1)
    dGam = z_eH / (2 * Gam)  # THIS ONLY HOLDS FOR THE ISOTROPIC CASE
    Rp, Rm, dRp, dRm = reflections(depth, e_zH, Gam, lrec, lsrc, ana_deriv=True, dGam=dGam, debug=False)

    Pu, Pd, dPu, dPd = fields(depth, Rp.copy(), Rm.copy(), Gam.copy(), lrec=lrec, lsrc=lsrc, zsrc=zsrc, ab=ab, TM=True,
                              ana_deriv=True, dRp=dRp.copy(), dRm=dRm.copy(), dGam=dGam.copy())
    ...
    for cond_idx in np.arange(cond.size):
        ...
        gamma2 = z_eH * e_zH
        Gam = np.sqrt(kappa ** 2 + gamma2).reshape(1, 1, -1, 1)
        Rp_pert, Rm_pert = reflections(depth, e_zH, Gam, lrec, lsrc, ana_deriv=False, debug=False)
        Pu_pert, Pd_pert = fields(depth, Rp_pert, Rm_pert, Gam, lrec=lrec, lsrc=lsrc, zsrc=zsrc, ab=ab, TM=True,
                                  ana_deriv=False, )
        ...
        gamma2 = z_eH * e_zH
        Gam = np.sqrt(kappa ** 2 + gamma2).reshape(1, 1, -1, 1)
        Rp_pert2, Rm_pert2 = reflections(depth, e_zH, Gam, lrec, lsrc, ana_deriv=False, debug=False)
        Pu_pert2, Pd_pert2 = fields(depth, Rp_pert2, Rm_pert2, Gam, lrec=lrec, lsrc=lsrc, zsrc=zsrc, ab=ab, TM=True,
                                    ana_deriv=False, )
...
def test_greenfct_jacobian_debug():
    ...
    gTM, gTE, dgTM, dgTE, gamTM, gamTE, dgamTM, dgamTE, Pu, dPu, Pd, dPd = greenfct(zsrc, zrec, lsrc, lrec, depth, etaH, etaV, zetaH,
                                                                  zetaV, lambd,
    ...
    gTMa, gTEa, dgTMa, dgTEa, gamTMa, gamTEa, dgamTMa, dgamTEa, Pupert, dPupert, Pdpert, dPdpert = greenfct(zsrc, zrec, lsrc, lrec, depth, etaH_pert,
                                                                                    etaH_pert, zetaH,
                                                                                    zetaV, lambd,
                                                                                    ab, xdirect, msrc, mrec,
                                                                                    ana_deriv=True, debug=True)
    assert_allclose(dPd[0, :, 0, cond_idx], ((Pdpert - Pd) / (h))[0, :,0], rtol=1e-6, atol=1e-9, )
    ...
    for cond_idx in np.arange(etaH.shape[1]):
        ...
        # Gamma
        assert_allclose(dgamTM[:, :, cond_idx, :20],
                        ((-gamTM1 + 8 * gamTM2 - 8 * gamTM3 + gamTM4) / (12 * h))[:, :, cond_idx, :20], rtol=1e-6,
                        atol=1e-8, )
        assert_allclose(dgamTE[:, :, cond_idx, :20],
                        ((-gamTE1 + 8 * gamTE2 - 8 * gamTE3 + gamTE4) / (12 * h))[:, :, cond_idx, :20], rtol=1e-6,
                        atol=1e-8, )

        # Green's function
        assert_allclose(dgTM[0, :, :20, cond_idx], ((-gTM1 + 8 * gTM2 - 8 * gTM3 + gTM4) / (12 * h))[0, :, :20],
                        rtol=1e-6, atol=1e-8, )
        assert_allclose(dgTE[0, 0, 0, cond_idx], ((-gTE1 + 8 * gTE2 - 8 * gTE3 + gTE4) / (12 * h))[0, 0, 0], rtol=1e-6,
                        atol=1e-8, )
